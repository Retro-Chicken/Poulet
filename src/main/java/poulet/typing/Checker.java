...
public class Checker {
    public static void checkType(Expression term, Expression type, Environment environment) throws PouletException {
        ...
        if (!Evaluator.convertible(deduced, type, environment)) {
            ...
            throw new PouletException(StringUtil.mapToStringWithNewlines(Map.of(
                    "term", term,
                    "deduced", deduced.normalizeSymbolNames(),
                    "type", type.normalizeSymbolNames()
            )));
        }
    }
    ...
    public static Expression deduceType(Expression term, Environment environment) throws PouletException {
        ...
        if (term instanceof Abstraction) {
            ...
            if (functionType instanceof PiType) {
                ...
                result = piType.body.substitute(piType.variable, application.argument);
            ...
            Expression bodyLevel = deduceType(piType.body.substitute(piType.variable, new Variable(tempSymbol)), newEnvironment);
            if (typeLevel instanceof Variable && bodyLevel instanceof Variable) {
                ...
                if (typeName.matches("Type\\d+") && bodyName.matches("Type\\d+")) {
                    ...
                    // TODO: why haven't we always been doing this? what am I missing?
                    result = new Variable(new Symbol("Type" + Math.max(level1, level2)));
                }
            }
            ...
            result = Evaluator.reduce(typeDeclaration.type, newEnvironment);
            ...
            result = Evaluator.reduce(constructor.definition, newEnvironment);
            //}
        ...
        if (result == null) {
            System.out.println(StringUtil.mapToStringWithNewlines(Map.of(
                    "term", term == null ? "null" : term,
                    "environment", environment
            )));
            throw new PouletException("Type Could not be Deduced");
        }
